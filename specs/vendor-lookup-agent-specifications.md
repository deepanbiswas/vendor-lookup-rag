...
## 1. Overview
This project automates the verification of vendor details extracted from invoices against a centralized vendor master database. The solution leverages local LLMs and a vector database for semantic search, ensuring high accuracy and data privacy without external API dependencies.

**Architecture:** For components (CSV importer, Qdrant, agent, retrieval tool, LLM, Streamlit) and the end-to-end execution flow, see [Vendor Lookup Agent Architecture](../docs/architecture.md).
...
* **Conversational Input:** The system must expose a Streamlit chat interface allowing users to input partial or full vendor details (e.g., name, city, VAT, ZIP).
...
* **Containerization:** The entire pipeline must be deployable via a single `docker-compose.yml` file.
