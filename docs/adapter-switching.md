...
This project uses **hexagonal-style ports** under [`src/vendor_lookup_rag/ports/`](../src/vendor_lookup_rag/ports/). Domain code (`retrieval`, `agent/runner` tool body) depends only on those protocols. **Composition roots** construct concrete adapters:
...
1. Add a module under `src/vendor_lookup_rag/adapters/<backend>/vector_store.py`.
2. Implement every method on [`VectorStore`](../src/vendor_lookup_rag/ports/vector_store.py):
...
1. Implement [`TextEmbedder`](../src/vendor_lookup_rag/ports/embedding.py) (`embed(text: str) -> list[float]`).
...
3. If the embedder holds connections, implement `close()`; [`ingest_vendor_csv`](../src/vendor_lookup_rag/ingestion/pipeline.py) calls `close()` when it created the embedder.
...
[`Settings`](../src/vendor_lookup_rag/config/settings.py) includes **backend discriminators** (single option each today; extend the `Literal` when you add another implementation):
...
- Domain records ([`VendorRecord`](../src/vendor_lookup_rag/models/records.py), tool result models) use Pydantic for validation and JSON compatibility with Qdrant payloads and tool schemas. Replacing that layer means rewriting validation and serialization across retrieval, ingestion, and the agent.
...
- Ports: [`src/vendor_lookup_rag/ports/`](../src/vendor_lookup_rag/ports/)
- Adapters: [`src/vendor_lookup_rag/adapters/`](../src/vendor_lookup_rag/adapters/)
- Factories: [`src/vendor_lookup_rag/adapters/factory.py`](../src/vendor_lookup_rag/adapters/factory.py)
- App logging: [`src/vendor_lookup_rag/observability/app_logging.py`](../src/vendor_lookup_rag/observability/app_logging.py)
- Test fakes: [`tests/fakes/`](../tests/fakes/)
